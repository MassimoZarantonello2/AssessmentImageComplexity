...
class slam(nn.Module):
    def __init__(self, spatial_dim):        #ritorna uno scalare compreso tra 0 e 1 che indica la complessità
        ...
        self.spatial_dim = spatial_dim      #dimensione della mappa di complessità
        self.linear = nn.Sequential(    #rete neurale che applica una trasformazione lineare all'immagine (Spatial Layout Attention Module)
...
class up_conv_bn_relu(nn.Module):
    def __init__(self,up_size, in_channels, out_channels = 64, kernal_size = 1, padding =0, stride = 1):
        ...
        self.upSample = nn.Upsample(size = (up_size,up_size),mode="bilinear",align_corners=True)    #viene eseguita una interpolazione bilineare
        self.conv = nn.Conv2d(in_channels=in_channels,out_channels=out_channels,kernel_size = kernal_size, stride = stride, padding= padding)   #viene eseguita una convoluzione
        self.bn = nn.BatchNorm2d(num_features=out_channels) #https://pytorch.org/docs/stable/generated/torch.nn.BatchNorm2d.html
        self.act = nn.ReLU()
...
class ICNet(nn.Module):    
    def __init__(self, is_pretrain = True, size1 = 512, size2 = 256):       #costruttore
        ...
        ## detail branch
        self.b1_1 = nn.Sequential(*list(resnet18Pretrained1.children())[:5])   #i primi 5 layer del modello preaddestrato vengono presi dal resnet18 e eseguiti sequenzialmente
        self.b1_1_slam = slam(32)   #creo il modulo di attenzione spaziale
    
        self.b1_2 = list(resnet18Pretrained1.children())[5]   #il quinto layer del modello preaddestrato viene preso dal resnet18
        self.b1_2_slam = slam(32)

        ## context branch
        self.b2_1 = nn.Sequential(*list(resnet18Pretrained2.children())[:5])    #stessa cosa ma resnet18 è addestrato su immagini di dimensione 256
        ...
        ## upsample
        self.upsize = size1 // 8    #dimensione immagine output = 64
    ...
    def forward(self,x1):
        ...
        score_feature = score_feature.squeeze()     #rimuove le dimensioni di lunghezza 1
